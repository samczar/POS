 ...
 * @version     2.1.0
 */
class WC_Pos_Report_Coupon_Usage extends WC_Admin_Report
{
    /**
     * Chart colors.
     *
    ...
    public function get_chart_legend()
    {
        ...
        $legend[] = array(
            ...
            'title' => sprintf(__('%s discounts in total', 'wc_pos'), '<strong>'.wc_price($total_discount).'</strong>'),
        ...
        $legend[] = array(
            ...
            'title' => sprintf(__('%s coupons used in total', 'wc_pos'), '<strong>'.$total_coupons.'</strong>'),
    ...
    {
        ?>
		<h4 class="section_title"><span><?php esc_html_e('Filter by coupon', 'wc_pos'); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
					<?php
        ...
        if (!empty($used_coupons) && is_array($used_coupons)) :
                    ?>
						<select id="coupon_codes" name="coupon_codes" class="wc-enhanced-select" data-placeholder="<?php esc_attr_e('Choose coupons&hellip;', 'wc_pos'); ?>" style="width:100%;">
							<option value=""><?php esc_html_e('All coupons', 'wc_pos'); ?></option>
							<?php
					...
					<?php else : ?>
						<span><?php esc_html_e('No used coupons found', 'wc_pos'); ?></span>
					<?php endif; ?>
				</div>
			</form>
		</div>
		<h4 class="section_title"><span><?php esc_html_e('Most popular', 'wc_pos'); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
        ...
        if (!empty($most_popular) && is_array($most_popular)) {
            ...
            echo '<tr><td colspan="2">'.esc_html__('No coupons found in range', 'wc_pos').'</td></tr>';
        } ?>
			</table>
		</div>
		<h4 class="section_title"><span><?php esc_html_e('Most discount', 'wc_pos'); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
        ...
        if (!empty($most_discount) && is_array($most_discount)) {
            ...
            echo '<tr><td colspan="3">'.esc_html__('No coupons found in range', 'wc_pos').'</td></tr>';
        } ?>
			</table>
		</div>
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						{
							label: "<?php echo esc_js(__('Number of coupons used', 'wc_pos')); ?>",
						...
						{
							label: "<?php echo esc_js(__('Discount amount', 'wc_pos')); ?>",
