...
class WC_Pos_Report_Low_In_Stock extends WC_Report_Stock
{
    ...
    public function get_items($current_page, $per_page, $outlet_id)
    {
        ...
        if ($outlet_id == 0) {
            $this->items = $wpdb->get_results($wpdb->prepare("SELECT posts.ID AS id, posts.post_parent AS parent FROM $wpdb->posts AS posts,
					 ...
					 (
						 (meta3.meta_key = '_pos_master_stock' AND meta3.meta_value <= $stock AND meta3.meta_value > 0)
						 OR
						 (meta3.meta_key = '_pos_variation_master_stock' AND meta3.meta_value <= $stock AND meta3.meta_value > 0)
					 )
					 GROUP BY posts.ID ORDER BY CAST(meta1.meta_value AS SIGNED) DESC LIMIT %d, %d", ($current_page - 1) * $per_page, $per_page));

            $this->max_items = $wpdb->get_var("SELECT COUNT(DISTINCT posts.ID) FROM $wpdb->posts AS posts,
			 $wpdb->postmeta as meta1 ,
			 $wpdb->postmeta as meta3
			 WHERE posts.ID = meta1.post_id AND
			 posts.ID = meta3.post_id AND
            ...
            $this->items = $wpdb->get_results($wpdb->prepare("SELECT posts.ID AS id, posts.post_parent AS parent FROM $wpdb->posts AS posts,
				 ...
				 (mapped.pos_stock <= $stock AND mapped.pos_stock > 0) AND
