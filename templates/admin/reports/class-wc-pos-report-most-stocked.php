 ...
 * @version     2.1.0
 */
class WC_Pos_Report_Most_Stocked extends WC_Report_Stock
{
    /**
    ...
    public function get_items($current_page, $per_page, $outlet_id)
    {
        ...
        if ($outlet_id == 0) {
            $this->items = $wpdb->get_results($wpdb->prepare("SELECT posts.ID AS id, posts.post_parent AS parent FROM $wpdb->posts AS posts,
					 ...
					 (
						 (meta3.meta_key = '_pos_master_stock' AND meta3.meta_value > $stock)
						 OR
						 (meta3.meta_key = '_pos_variation_master_stock' AND meta3.meta_value > $stock)
					 )
