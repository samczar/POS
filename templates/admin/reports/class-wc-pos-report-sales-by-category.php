 ...
 * @version     2.1.0
 */
class WC_Pos_Report_Sales_By_Category extends WC_Admin_Report
{
    /**
     * Chart colors.
     *
    ...
    public function get_chart_legend()
    {
        ...
        foreach ($this->show_categories as $category) {
            ...
            $legend[] = array(
                ...
                'title' => sprintf(__('%1$s sales in %2$s', 'wc_pos'), '<strong>'.wc_price($total).'</strong>', $category->name),
    ...
    public function category_widget()
    {
		...
		<form method="GET">
			<div>
				<select multiple="multiple" data-placeholder="<?php esc_attr_e('Select categories&hellip;', 'wc_pos'); ?>" class="wc-enhanced-select" id="show_categories" name="show_categories[]" style="width: 205px;">
					<?php
                        $r = array();
        $r['pad_counts'] = 1;
        $r['hierarchical'] = 1;
        ...
        echo wc_walk_category_dropdown_tree($categories, 0, $r); // @codingStandardsIgnoreLine
				...
				<a href="#" class="select_none"><?php esc_html_e('None', 'wc_pos'); ?></a>
				<a href="#" class="select_all"><?php esc_html_e('All', 'wc_pos'); ?></a>
    ...
    public function get_main_chart()
    {
        ...
        if (empty($this->show_categories)) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php esc_html_e('Choose a category to view stats', 'wc_pos'); ?></p>
			</div>
			<?php
        } else {
            $chart_data = array();
