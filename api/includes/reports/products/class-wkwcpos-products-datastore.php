...
namespace WKWC_POS\Api\Includes\Reports\Products;
 ...
 */
class WKWCPOS_Products_DataStore extends WKWCPOS_Report_DataStore {
	...
	protected function get_sql_query_params($query_args)
	{
		...
		if ($order_status_filter) {
			$sql_query_params['from_clause']  .= " JOIN {$wpdb->prefix}woocommerce_pos_order_stats ON {$order_product_lookup_table}.order_id = {$wpdb->prefix}woocommerce_pos_order_stats.order_id";
		...
		foreach ($products_data as $key => $product_data) {
			...
			if ($query_args['extended_info']) {
				...
				if (!$product) {
					...
					$products_data[$key]['extended_info']['name'] = $product_names[$product_id] ? sprintf(__('%s (Deleted)', 'wc_pos'), $product_names[$product_id]) : __('(Deleted)', 'wc_pos');
					continue;
				}
