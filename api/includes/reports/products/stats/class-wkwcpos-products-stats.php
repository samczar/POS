...
namespace WKWC_POS\Api\Includes\Reports\Products\Stats;

use WKWC_POS\Api\Includes\Reports\TimeInterval;
use WKWC_POS\Api\Includes\Reports\Products\WKWCPOS_Products_DataStore;
 ...
 */
class WKWCPOS_Products_Stats extends WKWCPOS_Products_DataStore {
	...
	protected function update_sql_query_params($query_args, &$totals_params, &$intervals_params)
	{
		...
		if ($order_status_filter) {
			$products_from_clause  .= " JOIN {$wpdb->prefix}woocommerce_pos_order_stats ON {$order_product_lookup_table}.order_id = {$wpdb->prefix}woocommerce_pos_order_stats.order_id";
	...
	public function get_items($query_args)
	{
		...
		if (false === $product_data) {
			...
			if (null === $totals) {
				return new \WP_Error('woocommerce_reports_products_stats_result_failed', __('Sorry, fetching revenue data failed.', 'wc_pos'));
			}
