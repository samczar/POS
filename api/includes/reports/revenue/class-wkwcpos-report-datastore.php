<?php
/**
 * WooCommerce Point of Sale\API\Reports\DataStore class file.
 *
 * @package WooCommerce Point of Sale/Classes
 */

namespace WKWC_POS\Api\Includes\Reports\Revenue;
...
use WKWC_POS\Api\WKWCPOS_Sql_Query;

/**
 * WooCommerce Point of Sale\API\Reports\DataStore: Common parent for custom report data stores.
 */
class WKWCPOS_Report_DataStore {
	 ...
	 */
	protected function update_intervals_sql_params( &$intervals_query, &$query_args, $db_interval_count, $expected_interval_count, $table_name, $payment = false ) {

		$date_column = $payment ? 'date' : 'date_created';
		...
		if ( 'date' === strtolower( $query_args['orderby'] ) ) {
			...
			$intervals_query['where_time_clause'] .= " AND {$table_name}.$date_column <= '$adj_before'";
			$intervals_query['where_time_clause'] .= " AND {$table_name}.$date_column >= '$adj_after'";
	 ...
	 * @return array
	*/
	protected function get_time_period_sql_params( $query_args, $table_name, $payment = false ) {
		...
		if( $payment ) {
			$sql_query['where_clause'] = ' AND outlet_id = ' . $query_args['outlet_id'];
		} else {
			$sql_query['where_clause'] = ' AND outlet_id = ' . $query_args['outlet_id'];
		}
		...
		if ( isset( $query_args['before'] ) && '' !== $query_args['before'] ) {
			...
			$sql_query['where_time_clause'] .= " AND {$table_name}.$date_column <= '$datetime_str'";
	...
	*/
	 ...
	 * @return array
	 */
	protected function get_intervals_sql_params( $query_args, $table_name, $payment = false ) {
		...
		if ($payment) {
			$intervals_query['where_clause'] = ' AND outlet_id = ' . $query_args['outlet_id'];
		} else {
			$intervals_query['where_clause'] = ' AND outlet_id = ' . $query_args['outlet_id'];
		}

		$intervals_query = array_merge( $intervals_query, $this->get_time_period_sql_params( $query_args, $table_name, $payment ) );

		if ( isset( $query_args['interval'] ) && '' !== $query_args['interval'] ) {
			$interval                         = $query_args['interval'];
			$intervals_query['select_clause'] = self::db_datetime_format( $interval, $table_name, $date_column );
		}
	 ...
	 * @return mixed
	 */
	public static function db_datetime_format($time_interval, $table_name, $date_column)
	{
		$first_day_of_week = absint(get_option('start_of_week'));

		if (1 === $first_day_of_week) {
			// Week begins on Monday, ISO 8601.
			$week_format = "DATE_FORMAT({$table_name}.$date_column, '%x-%v')";
		} else {
			// Week begins on day other than specified by ISO 8601, needs to be in sync with function simple_week_number.
			$week_format = "CONCAT(YEAR({$table_name}.$date_column), '-', LPAD( FLOOR( ( DAYOFYEAR({$table_name}.$date_column) + ( ( DATE_FORMAT(MAKEDATE(YEAR({$table_name}.$date_column),1), '%w') - $first_day_of_week + 7 ) % 7 ) - 1 ) / 7  ) + 1 , 2, '0'))";
		}
		...
		$mysql_date_format_mapping = array(
			'hour'    => "DATE_FORMAT({$table_name}.$date_column, '%Y-%m-%d %H')",
			'day'     => "DATE_FORMAT({$table_name}.$date_column, '%Y-%m-%d')",
			'week'    => $week_format,
			'month'   => "DATE_FORMAT({$table_name}.$date_column, '%Y-%m')",
			'quarter' => "CONCAT(YEAR({$table_name}.$date_column), '-', QUARTER({$table_name}.$date_column))",
			'year'    => "YEAR({$table_name}.$date_column)",
	...
	protected function get_refund_subquery( $query_args ) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'woocommerce_pos_order_stats';
		$sql_query  = array(
			'where_clause' => '',
			'from_clause'  => '',
		);
	...
	protected function get_status_subquery( $query_args, $operator = 'AND' ) {
		...
		if ( isset( $query_args['status_is'] ) && is_array( $query_args['status_is'] ) && count( $query_args['status_is'] ) > 0 ) {
			...
			if ( $allowed_statuses ) {
				$subqueries[] = "{$wpdb->prefix}woocommerce_pos_order_stats.status IN ( '" . implode( "','", $allowed_statuses ) . "' )";
			}
		}
		...
		if ( $excluded_statuses ) {
			$subqueries[] = "{$wpdb->prefix}woocommerce_pos_order_stats.status NOT IN ( '" . implode( "','", $excluded_statuses ) . "' )";
		}
	...
	protected function get_customer_subquery( $query_args ) {
		...
		if ( isset( $query_args['customer'] ) ) {
			if ( 'new' === strtolower( $query_args['customer'] ) ) {
				$customer_filter = " {$wpdb->prefix}woocommerce_pos_order_stats.returning_customer = 0";
			} elseif ( 'returning' === strtolower( $query_args['customer'] ) ) {
				$customer_filter = " {$wpdb->prefix}woocommerce_pos_order_stats.returning_customer = 1";
			}
		}
