...
namespace WKWC_POS\Api\Includes\Reports\Taxes\Stats;

use WKWC_POS\Api\Includes\Reports\TimeInterval;
use WKWC_POS\Api\Includes\Reports\Taxes\WKWCPOS_Taxes_DataStore;
 ...
 */
class WKWCPOS_Taxes_Stats extends WKWCPOS_Taxes_DataStore {
	...
	public function get_items( $query_args ) {
		...
		if (false === $data) {
			...
			$order_stats_join = "JOIN {$wpdb->prefix}woocommerce_pos_order_stats ON {$table_name}.order_id = {$wpdb->prefix}woocommerce_pos_order_stats.order_id";
			...
			if ( null === $totals ) {
				return new \WP_Error( 'woocommerce_analytics_taxes_stats_result_failed', __( 'Sorry, fetching revenue data failed.', 'wc_pos' ) );
			}
			...
			if ( null === $intervals ) {
				return new \WP_Error( 'woocommerce_analytics_taxes_stats_result_failed', __( 'Sorry, fetching tax data failed.', 'wc_pos' ) );
			}
	...
	protected function initialize_queries() {
		...
		$this->total_query = new WKWCPOS_Sql_Query( $this->context . '_total' );
		$this->total_query->add_sql_clause( 'from', $wpdb->prefix . self::TABLE_NAME );

		$this->interval_query = new WKWCPOS_Sql_Query( $this->context . '_interval' );
