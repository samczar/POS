	...
	public static function rest_validate_between_numeric_arg( $value, $request, $param ) {
		if ( ! wp_is_numeric_array( $value ) ) {
			return new \WP_Error(
				...
				sprintf( __( '%1$s is not a numerically indexed array.', 'wc_pos' ), $param )
			);
		}
		...
		if (
			...
			return new \WP_Error(
				...
				sprintf( __( '%s must contain 2 numbers.', 'wc_pos' ), $param )
			);
		}
	...
	public static function rest_validate_between_date_arg( $value, $request, $param ) {
		...
		if (
			...
			return new \WP_Error(
				'rest_invalid_param',
				/* translators: %s: parameter name */
				sprintf( __( '%s must contain 2 valid dates.', 'wc_pos' ), $param )
			);
		}
